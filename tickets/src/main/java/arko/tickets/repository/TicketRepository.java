...
public interface TicketRepository extends JpaRepository<Ticket, Long> {
    ...
    @Query("SELECT t FROM Ticket t WHERE LOWER(t.descripcion) LIKE LOWER(CONCAT('%', :descripcion, '%'))")
    List<Ticket> findByDescripcionContaining(@Param("descripcion") String descripcion);

    @Query("SELECT t FROM Ticket t WHERE t.estado = :estado AND t.prioridad = :prioridad ORDER BY t.id DESC")
    List<Ticket> findByEstadoAndPrioridad(@Param("estado") String estado, @Param("prioridad") String prioridad);

    @Query("SELECT t FROM Ticket t WHERE t.responsable = :responsable AND t.estado = :estado ORDER BY t.id DESC")
