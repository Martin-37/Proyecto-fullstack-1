...
import arko.tickets.webclient.DocumentosClient;
...
public class TicketService {
    ...
    public TicketResponseDTO guardar(TicketRequestDTO requestDTO) {
        ...
        ticket.setDocumentoId(requestDTO.getDocumentoId());
    ...
    // Puente con documentos-service: devuelve el ticket + su documento asociado
    public Map<String, Object> obtenerConDocumento(Long id, String token) {
        Ticket ticket = ticketRepository.findById(id)
                .orElseThrow(() -> new RuntimeException("Ticket no encontrado con ID: " + id));

        Map<String, Object> resultado = new HashMap<>();
        resultado.put("id", ticket.getId());
        resultado.put("titulo", ticket.getTitulo());
        resultado.put("descripcion", ticket.getDescripcion());
        resultado.put("prioridad", ticket.getPrioridad());
        resultado.put("estado", ticket.getEstado());
        resultado.put("responsable", ticket.getResponsable());
        resultado.put("documentoId", ticket.getDocumentoId());

        Map<String, Object> documento = null;
        if (ticket.getDocumentoId() != null) {
            documento = documentosClient.obtenerDocumento(ticket.getDocumentoId(), token);
        }
